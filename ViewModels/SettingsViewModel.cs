...
public partial class SettingsViewModel : ObservableObject
{
    ...
    private void DetectAndSelectDefaultGamepad()
    {
        ...
        else if (foundVid.HasValue)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"[SettingsVM] Detected gamepad differs from saved one. Keeping saved selection.");
        }
        else
        {
    ...
    public void ApplyKeyboardCapture(string displayValue, object dataValue)
    {
        ...
        foreach (var existingBinding in Bindings)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[SettingsVM] Clearing previous keyboard binding '{displayValue}' from action '{existingBinding.ActionId}'");
        ...
        if (internalButtonCode != null)
        {
            ...
            foreach (var existingBinding in Bindings)
            {
                if (existingBinding != BindingToCapture &&
                    ...
                    existingBinding.JoystickBindingDisplay.Equals(internalButtonCode, StringComparison.OrdinalIgnoreCase))
                {
                    System.Diagnostics.Debug.WriteLine($"[SettingsVM] Clearing previous joystick binding '{internalButtonCode}' from action '{existingBinding.ActionId}'");
    ...
    private void ResetKeyboardDefaults()
    {
        ...
        foreach (var binding in Bindings)
        {
            ...
            if (defaultDict.TryGetValue(binding.ActionId, out string? defaultKeyBinding))
            {
                binding.KeyboardBindingDisplay = defaultKeyBinding;
                if (defaultKeyBinding != null) System.Diagnostics.Debug.WriteLine($"[SettingsVM] Reset Key '{binding.ActionId}' to: {defaultKeyBinding}");
            }
            else
            {
                binding.KeyboardBindingDisplay = null;
                System.Diagnostics.Debug.WriteLine($"[SettingsVM] Cleared Key '{binding.ActionId}' (no default).");
            }
        }
    ...
    private void ResetJoystickDefaults()
    {
        ...
        foreach (var binding in Bindings)
        {
            ...
            if (defaultDict.TryGetValue(binding.ActionId, out string? defaultJoyBinding))
            {
                binding.JoystickBindingDisplay = defaultJoyBinding;
                if (defaultJoyBinding != null) System.Diagnostics.Debug.WriteLine($"[SettingsVM] Reset Joy '{binding.ActionId}' to: {defaultJoyBinding}");
            }
            else
            {
                binding.JoystickBindingDisplay = null;
                System.Diagnostics.Debug.WriteLine($"[SettingsVM] Cleared Joy '{binding.ActionId}' (no default).");
            }
        }
    ...
    private void SaveAndClose()
    {
        ...
        foreach (var binding in Bindings)
        {
            ...
            if (!string.IsNullOrEmpty(binding.JoystickBindingDisplay))
                joyTargetDict[binding.ActionId] = binding.JoystickBindingDisplay;

            if (!string.IsNullOrEmpty(binding.KeyboardBindingDisplay))
                keyTargetDict[binding.ActionId] = binding.KeyboardBindingDisplay;
        }
...
{
    public const int NINTENDO_VID = 0x057E;
    public const int PRO_CONTROLLER_PID_USB = 0x2009;
    public const int XBOX_VID = 0x045E;
    public const int XBOX_PID = 0x028E;
}