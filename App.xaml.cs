...
namespace JpnStudyTool
{
    public partial class App : Application
    {
        ...
        internal static void ReloadAndRestartGlobalServices(HWND? hwndCheck = null, AppSettings? settings = null)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"[App] Reloaded Settings - VID:{targetVid?.ToString("X4") ?? "N"}, PID:{targetPid?.ToString("X4") ?? "N"}, ToggleJoy:'{toggleJoyCode ?? "N"}', MenuJoy:'{menuJoyCode ?? "N"}'");
            ...
            System.Diagnostics.Debug.WriteLine($"[App] Reloaded Settings - ToggleKey:'{toggleKeyStr ?? "N"}', MenuKey:'{menuKeyStr ?? "N"}'");
            if (currentHwnd != default && currentHwnd != HWND.Null)
            {
                ...
                try
                {
                    ...
                    _keyboardHotkeyService.RegisterHotkeys(currentSettings.GlobalKeyboardBindings);
                    System.Diagnostics.Debug.WriteLine("[App] KeyboardHotkeyService configured and keys registered.");
                }
                catch (Exception ex)
                {
        ...
        public static void ResumeGlobalHotkeys()
        {
            ...
            if (_keyboardHotkeyService != null && _settingsService != null)
            {
                try
                {
                    ...
                    _keyboardHotkeyService.RegisterHotkeys(settings.GlobalKeyboardBindings);
